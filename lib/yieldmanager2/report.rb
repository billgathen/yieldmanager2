...
module Yieldmanager2
  ...
  #
  # The #pull method is typically called by Yieldmanager2::Client#pull_report.
  #
  ...
  class Report
    ...
    def pull token, service, xml
      report_token = request_report_token token, service, xml
      report_url = retrieve_report_url token, service, report_token
...
private
    ...
    def request_report_token token, service, xml
      service.requestViaXML(token,xml)
    end

    def retrieve_report_url token, service, report_token
      report_url = nil
      60.times do |secs| # Poll until report ready
        report_url = service.status(token,report_token)
    ...
    def retrieve_data url
      ...
      doc.css("header column").each do |col|
        headers << col.content
      end
